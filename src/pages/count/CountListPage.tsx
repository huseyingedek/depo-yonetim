    ...
    <div className="mx-auto max-w-6xl p-4 lg:p-8">
      ...
      {loading ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {[0, 1, 2].map((i) => <div key={i} className="h-24 animate-pulse rounded-2xl bg-elevated" />)}
        </div>
      ) : filtered.length === 0 ? (
            ...
            {pg.pageItems.map((c) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-mono font-bold text-fg">{c.location}</p>
                  <p className="text-xs text-subtle">{c.lines.length} {t("count.items")} · {c.reference}</p>
                </div>
